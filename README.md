...
| 역할/영역 | 담당 내용 |
|---|---|
| 🗂 DB 설계 | 게시판(Board) 관련 테이블 및 첨부파일 구조 설계 |
| 📌 게시판 기능 | 목록/검색/페이징 통합, CRUD, 다중 파일 업로드, 권한 기반 접근 제어 |
| ⭐ 추천 도서 기능 | 로그인 여부 및 구매 이력 기반 추천 도서 로직 구현 |
| 🎨 프론트엔드 UI 구현 | JSP 기반 사용자 화면 레이아웃 및 페이지 UI 구성 |
---
...
✔️ 게시글 수 증가에 대비하여 목록 조회, 검색, 페이징이 함께 동작하는 게시판 구조를 설계하고 구현  
...
**성과**: 
검색 여부와 관계없이 일관된 사용자 경험을 제공하고, 대량 데이터 환경에서도 안정적인 목록 조회 구조를 확보
...
✔️ 검색 조건에 따라 SQL이 동적으로 변경되도록 MyBatis 동적 SQL 기반 검색 로직을 구현
...
**성과**:
중복 쿼리 없이 유연한 검색 구조를 구성하여 유지보수성을 향상
...
✔️ 게시판 CRUD 전반을 풀스택으로 구현하고, 다중 이미지 첨부 기능을 함께 적용 
  
**동작 흐름**:
게시글 작성 → 파일 업로드 → 날짜별 디렉토리 저장 → DB 메타데이터 저장 → 상세 페이지 출력
...
**성과**:
실제 서비스에서 사용 가능한 안정적인 파일 관리 게시판 구조를 구현

### 4. 게시글 수정·삭제 권한 제어
✔️ 게시글 수정 및 삭제는 작성자 또는 관리자만 가능하도록 권한을 제한
- 세션 기반 로그인 사용자 검증
- 작성자 본인 여부 확인
...
**성과**:
권한 없는 접근을 방지하여 게시판 데이터 안정성과 신뢰성을 확보

### 5. 메인 페이지 추천 도서 기능 구현
✔️ 메인 페이지에서 사용자 상태에 따라 추천 도서 로직이 다르게 동작하도록 구현
...
**성과**:
사용자 상태를 고려한 추천 로직으로 서비스 개인화 경험을 강화
...
아래 영상에서는 온라인 북스토어(책갈피) 주요 기능의 동작 방식과 서비스 흐름을 실제 구동 화면으로 확인하실 수 있습니다.
